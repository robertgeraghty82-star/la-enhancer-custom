...
function loadDefaultProfile() {
    if (window.top.$.jStorage.get(sKey("profile:" + profile_10)) == null) {
        window.top.$.jStorage.set(sKey("profile:" + profile_10), ["1", "1", "distance", "asc", false, false, false, false, false, false, false, false, "hide", "", false, false, false, false, "greater_than", "", false, "greater_than", "", false, "greater_than", "", false, "greater_than", "", "hide", "", false, "hide", "", false, false, false, "", false, "", false, false, false]);
...
function getAttackTier(row, profileArray) {
    ...
    var firstBracket = 160;
    var bracket = 80;
    var maxAttacks = 100;
    var tier;
    if (total <= firstBracket) {
        tier = 1;
    } else {
        tier = 1 + Math.ceil((total - firstBracket) / bracket);
    }
    if (tier > maxAttacks) {
        tier = maxAttacks;
    }
    ...
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            ...
            if (button.hasClass("farm_icon_disabled") || button.html() == undefined) {
                ...
                var existing = getCurrentOutgoingAttacks(row);
                if (target <= 0 || existing >= target) {
                    row.hide();
                } else {
                    button.click();
                    doTime(200);
