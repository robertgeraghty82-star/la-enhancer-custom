...
var keyPressSettings = {
    ...
    "left_char": "ÃÂ¢Ã¢â¬ ÃÂ",
    "right_code": 39,
    "right_char": "ÃÂ¢Ã¢â¬ Ã¢â¬â¢",
    "priorities": [
        {"enabled": true, "profile": "Default", "button": "Skip"},
        {"enabled": true, "profile": "Default", "button": "Skip"},
        {"enabled": true, "profile": "Default", "button": "Skip"}
    ],
    "defaultButton": "Skip"
...
window.top.$.getScript(scriptURL + 'lib/jstorage.js', function () {
    window.top.$.getScript(scriptURL + "resources.js", function () {
        if (window.top.$.jStorage.get(sKey("language")) == null) {
            setDefaultLanguage();
        }
        window.top.$.getScript(scriptURL + "lang/" + window.top.$.jStorage.get(sKey("language")) + '.js', function () {
            console.log("init");
            checkPage();
        });
    });
...
function checkVersion() {
    if (getVersion() != version) {
        ...
        if (clearProfiles) {
            var profileList = window.top.$.jStorage.get(sKey("profileList"));
            window.top.$.each(profileList, function (i, val) {
                window.top.$.jStorage.deleteKey(sKey("profile:" + val));
            });
            window.top.$.jStorage.set(sKey("keyPressSettings"), keyPressSettings);
...
function checkWorking() {
    var acknowledged = window.top.$.jStorage.get(sKey("working"));
    if (acknowledged == null) {
        window.top.$.jStorage.set(sKey("working"), false);
    }
    if (getVersion() != version) {
        window.top.$.jStorage.set(sKey("working"), false);
    }
    if (working == false && acknowledged == false) {
        ...
        window.top.$.jStorage.set(sKey("working"), true);
    }
}
function setVersion() {
    window.top.$.jStorage.set(sKey("version"), version);
}
function getVersion() {
    var ver = window.top.$.jStorage.get(sKey("version"));
    if (ver == undefined) {
        setVersion();
        return version;
    }
...
function sortHaulRows() {
    if (!userset[s.sort_full_first]) {
        return;
    }
    var $table = window.top.$('#plunder_list');
    var $rows = $table.find('tr:gt(0)').get();
    $rows.sort(function (a, b) {
        return getHaulRank(a) - getHaulRank(b);
    });
    window.top.$.each($rows, function (i, row) {
        $table.append(row);
    });
}
function getHaulRank(row) {
    var haulCell = window.top.$(row).find('td').filter(function () {
        var html = window.top.$(this).html();
        return html && html.indexOf('max_loot') >= 0;
    }).eq(0);
    if (haulCell.length && haulCell.html().indexOf('max_loot/1') >= 0) {
        return 0;
    }
    return 1;
}
...
function setLocalStorageRow(village) {
    var localTitle = sKey("sitter:" + sitter + ", village:" + village + ", world:" + getURL()[0]);
...
function hideRecentlyFarmed(cell, profileArray) {
    if (profileArray[s.hide_recent_farms]) {
        ...
        localTitle = sKey("sitter:" + sitter + ", village:" + village + ", world:" + getURL()[0]);
    ...
    window.top.$('#am_widget_Farm tr:gt(0)').each(function (i) {
        window.top.$(this).children("td").each(function (j) {
            if (j == 4) {
                ...
                localTitle = sKey("sitter:" + sitter + ", village:" + reportLinkText + ", world:" + getURL()[0]);
...
function loadDefaultProfile() {
    if (window.top.$.jStorage.get(sKey("profile:" + profile_10)) == null) {
        window.top.$.jStorage.set(sKey("profile:" + profile_10), ["1", "1", "distance", "asc", false, false, false, false, false, false, false, false, "hide", "", false, false, false, false, "greater_than", "", false, "greater_than", "", false, "greater_than", "", false, "greater_than", "", "hide", "", false, "hide", "", false, false, false, "", false, "", false, false, false, "80", "10"]);
        window.top.$.jStorage.deleteKey(sKey("profileList"));
        window.top.$.jStorage.set(sKey("profileList"), [profile_10]);
    }
    userset = window.top.$.jStorage.get(sKey("profile:" + profile_10));
...
function setDefaultProfile() {
    if (window.top.$('#settingsProfile').val() == profile_10) {
        ...
        var profile = window.top.$.jStorage.get(sKey("profile:" + window.top.$('#settingsProfile').val()));
        window.top.$.jStorage.set(sKey("profile:" + profile_10), profile);
    }
}
function fillProfileList() {
    ...
    window.top.$('#settingsProfile').val(window.top.$.jStorage.get(sKey("DefaultProfile")));
}
function createProfile() {
    var profileName = prompt(dialog_03 + ":");
    if (window.top.$.inArray(profileName, window.top.$.jStorage.get(sKey("profileList"))) != -1) {
        alert(dialog_04);
        createProfile();
        return false;
    }
    ...
    if (profileName != null && profileName != "") {
        ...
        settings.push(window.top.$('#sort_full_first').prop('checked'));
        settings.push(window.top.$('#tiered_enable').prop('checked'));
        settings.push(window.top.$('#tiered_bracket').val());
        settings.push(window.top.$('#tiered_max').val());
        window.top.$.jStorage.set(sKey("profile:" + profileName), settings);
        var profileList = window.top.$.jStorage.get(sKey("profileList"));
        profileList.push(profileName);
        window.top.$.jStorage.set(sKey("profileList"), profileList)
...
function loadProfile(profile) {
    var settings = window.top.$.jStorage.get(sKey("profile:" + profile));
    userset = settings;
    ...
    window.top.$('#sort_full_first').prop('checked', settings[41]);
    window.top.$('#tiered_enable').prop('checked', settings[42]);
    window.top.$('#tiered_bracket').val(settings[43]);
    window.top.$('#tiered_max').val(settings[44]);
    window.top.$('#tiered_enable').prop('checked', settings[42]);
    window.top.$('#tiered_bracket').val(settings[43]);
    window.top.$('#tiered_max').val(settings[44]);
}
...
function deleteProfile() {
    ...
    if (profileName == profile_10) {
        ...
        var profilesList = window.top.$.jStorage.get(sKey("profileList"));
        profilesList.splice(profilesList.indexOf(profileName), 1);
        window.top.$.jStorage.set(sKey("profileList"), profilesList);
        window.top.$.jStorage.deleteKey(sKey("profile:" + profileName));
...
function exportProfile() {
    var profileName = window.top.$('#settingsProfile').val();
    var settings = window.top.$.jStorage.get(sKey("profile:" + profileName));
    if (profileName == profile_10) {
        alert(dialog_07);
    } else {
...
function importProfile() {
    ...
    if (window.top.$.inArray(profileName, profileList) != -1) {
        ...
        window.top.$.jStorage.set(sKey("profile:" + profileName), profileSettings);
        profileList.push(profileName);
        window.top.$.jStorage.set(sKey("profileList"), profileList);
        window.top.$('#settingsProfile').append("<option value='" + profileName + "'>" + profileName + "</option>");
        window.top.$('#settingsProfile').val(profileName);
...
function getAttackTier(row, profileArray) {
    var cell = row.children("td").eq(6);
    var total = 0;
    if (cell.length) {
        if (window.top.$.trim(cell.find('span').html()) == "?") {
            total = 0;
        } else {
            var wood = parseInt(cell.children('span').eq(0).html().replace(/\D+/g, '')) || 0;
            var clay = parseInt(cell.children('span').eq(1).html().replace(/\D+/g, '')) || 0;
            var iron = parseInt(cell.children('span').eq(2).html().replace(/\D+/g, '')) || 0;
            total = wood + clay + iron;
        }
    }
    var bracket = parseInt(profileArray[s.tiered_bracket]) || 80;
    var maxAttacks = parseInt(profileArray[s.tiered_max]) || 10;
    if (total <= bracket) {
        return 0;
    }
    var tier = Math.ceil(total / bracket) - 1;
    if (tier > maxAttacks) {
        tier = maxAttacks;
    }
    return tier;
...
function editKey(e) {
    if ((e.keyCode <= 37 && e.keyCode >= 40) || (e.keyCode <= 48 && e.keyCode >= 90)) {
        ...
        if (e.keyCode == 38) {
            keyToChar = "ÃÂ¢Ã¢â¬ Ã¢â¬Ë";
        }
        if (e.keyCode == 39) {
            keyToChar = "ÃÂ¢Ã¢â¬ Ã¢â¬â¢";
        }
        if (e.keyCode == 40) {
            keyToChar = "ÃÂ¢Ã¢â¬ Ã¢â¬Å";
        }
        switch (keyToEdit) {
            case"A":
...
function updateKeypressSettings() {
    ...
    var priorities = [];
    window.top.$('#priorityRowsInner .priorityRow').each(function () {
        priorities.push({
            enabled: window.top.$(this).find('.priorityEnabled').prop('checked'),
            profile: window.top.$(this).find('.priorityProfile').val(),
            button: window.top.$(this).find('.priorityButton').val()
        });
    });
    if (priorities.length) {
        keyPressSettings.priorities = priorities;
    }
    keyPressSettings.defaultButton = window.top.$('#defaultButton').val();
    window.top.$.jStorage.set(sKey("keyPressSettings"), keyPressSettings);
}
function fillKeypressSettings() {
    if (window.top.$.jStorage.get(sKey('keyPressSettings')) == null) {
        window.top.$.jStorage.set(sKey('keyPressSettings'), keyPressSettings);
    }
    keyPressSettings = window.top.$.jStorage.get(sKey('keyPressSettings'));
    if (!keyPressSettings.priorities) {
        keyPressSettings.priorities = [
            {enabled: keyPressSettings.priorityOneEnabled, profile: keyPressSettings.priorityOneProfile, button: keyPressSettings.priorityOneButton},
            {enabled: keyPressSettings.priorityTwoEnabled, profile: keyPressSettings.priorityTwoProfile, button: keyPressSettings.priorityTwoButton},
            {enabled: keyPressSettings.priorityThreeEnabled, profile: keyPressSettings.priorityThreeProfile, button: keyPressSettings.priorityThreeButton}
        ];
        window.top.$.jStorage.set(sKey('keyPressSettings'), keyPressSettings);
    }
...
function renderPriorityRows() {
    var container = window.top.$('#priorityRowsInner');
    container.empty();
    var profileList = window.top.$.jStorage.get(sKey("profileList"));
    window.top.$.each(keyPressSettings.priorities, function (i, p) {
        var row = window.top.$("<div class='priorityRow' style='margin-bottom:3px;'></div>");
        row.append("<input type='checkbox' class='priorityEnabled' " + (p.enabled ? "checked" : "") + " onchange='return updateKeypressSettings();'/> ");
        row.append(filter_54 + " ");
        var profileSelect = window.top.$("<select class='priorityProfile' onchange='return updateKeypressSettings();'></select>");
        window.top.$.each(profileList, function (j, val) {
            profileSelect.append("<option value='" + val + "'" + (val === p.profile ? " selected" : "") + ">" + val + "</option>");
        });
        row.append(profileSelect);
        row.append(" " + filter_55 + " ");
        var buttonSelect = window.top.$("<select class='priorityButton' onchange='return updateKeypressSettings();'></select>");
        window.top.$.each([filter_56, filter_57, filter_58, filter_59], function (j, val) {
            buttonSelect.append("<option value='" + val + "'" + (val === p.button ? " selected" : "") + ">" + val + "</option>");
        });
        row.append(buttonSelect);
        if (keyPressSettings.priorities.length > 1) {
            row.append(" <a href='#' class='removePriority' onclick='return removePriorityRow(" + i + ");'>&times;</a>");
        }
        container.append(row);
    });
    window.top.$('#priorityCount').text(keyPressSettings.priorities.length);
    if (keyPressSettings.priorities.length >= MAX_PRIORITIES) {
        window.top.$('#addPriorityLink').hide();
    } else {
        window.top.$('#addPriorityLink').show();
    }
}
function addPriorityRow() {
    if (keyPressSettings.priorities.length < MAX_PRIORITIES) {
        keyPressSettings.priorities.push({enabled: true, profile: "Default", button: "Skip"});
        renderPriorityRows();
        updateKeypressSettings();
    }
    return false;
...
function removePriorityRow(index) {
    keyPressSettings.priorities.splice(index, 1);
    renderPriorityRows();
    updateKeypressSettings();
    return false;
}
function selectMasterButton(row) {
    var buttonToClick = keyPressSettings.defaultButton;
    var aButton = row.children("td").eq(9).children("a");
    var bButton = row.children("td").eq(10).children("a");
    var cButton = row.children("td").eq(11).children("a");
    for (var i = keyPressSettings.priorities.length - 1; i >= 0; i--) {
        var p = keyPressSettings.priorities[i];
        if (p.enabled) {
            var profileSettings = window.top.$.jStorage.get(sKey("profile:" + p.profile));
            if (profileSettings && !checkRowToHide(row, profileSettings)) {
                buttonToClick = p.button;
            }
        }
    }
...
function setDefaultLanguage() {
    ...
    switch (domain) {
        case"fyletikesmaxes.gr":
            window.top.$.jStorage.set(sKey("language"), "el");
            break;
        case"tribals.it":
            window.top.$.jStorage.set(sKey("language"), "it");
            break;
        case"guerrastribales.es":
            window.top.$.jStorage.set(sKey("language"), "es");
            break;
        case"tribalwars.ae":
            window.top.$.jStorage.set(sKey("language"), "ar");
            break;
        case"divoke-kmene.sk":
            window.top.$.jStorage.set(sKey("language"), "sk");
            break;
        default:
            window.top.$.jStorage.set(sKey("language"), "en");
            break;
    }
}
function loadLanguage(lang) {
    window.top.$.jStorage.set(sKey("language"), lang);
    var profileList = window.top.$.jStorage.get(sKey("profileList"));
    var defaultProfile = window.top.$.jStorage.get(sKey("profile:" + profile_10));
    ...
    window.top.$.getScript(langFile, function () {
        ...
        window.top.$.jStorage.set(sKey("profileList"), profileList);
        window.top.$.jStorage.set(sKey("profile:" + profile_10), defaultProfile);
        changeHeader(filter_40);
        showSettings();
    });
}
function addLanguages() {
    ...
    window.top.$('#language').append("<option value='el'>ÃÅ½Ã¢â¬Â¢ÃÅ½ÃÂ»ÃÅ½ÃÂ»ÃÅ½ÃÂ·ÃÅ½ÃÂ½ÃÅ½ÃÂ¹ÃÅ½ÃÂºÃÅ½ÃÂ¬</option>");
    window.top.$('#language').append("<option value='it'>Italiano</option>");
    window.top.$('#language').append("<option value='es'>EspaÃÆÃÂ±ol</option>");
    window.top.$('#language').append("<option value='ar'>ÃËÃÂ§Ãâ¢Ã¢â¬Å¾Ãâ¢Ã¢â¬Å¾ÃËÃÂºÃËÃÂ© ÃËÃÂ§Ãâ¢Ã¢â¬Å¾ÃËÃÂ¹ÃËÃÂ±ÃËÃÂ¨Ãâ¢Ã ÃËÃÂ©</option>");
    window.top.$('#language').append("<option value='sk'>SlovenÄina</option>");
}
