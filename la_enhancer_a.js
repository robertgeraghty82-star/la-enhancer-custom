...
function tryClick(button) {
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            var smartRow = null;
            var isFirstOfPair = null;
            if (userset[s.smart_ab]) {
                smartRow = button.closest('tr');
                var villageKey = smartRow.attr('name');
                var aButton = smartRow.children("td").eq(9).children("a");
                var bButton = smartRow.children("td").eq(10).children("a");
                isFirstOfPair = !abSentTracker[villageKey];
                button = isFirstOfPair ? aButton : bButton;
            }
            ...
            if (button.hasClass("farm_icon_disabled") || button.html() == undefined) {
                ...
                button.closest('tr').hide();
                if (userset[s.smart_ab]) {
                    delete abSentTracker[smartRow.attr('name')];
                }
            }
            else {
                button.click();
                if (userset[s.smart_ab]) {
                    var villageKeyAfter = smartRow.attr('name');
                    if (isFirstOfPair) {
                        abSentTracker[villageKeyAfter] = true;
                        setTimeout(function () {
                            smartRow.show();
                        }, 300);
                    } else {
                        delete abSentTracker[villageKeyAfter];
                    }
                }
