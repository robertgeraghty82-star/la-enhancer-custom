    ...
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            if (userset[s.smart_ab]) {
                ...
                var lightAvailable = units && units.light >= 1;
                var heavyAvailable = units && units.heavy >= 1;
                if (lightAvailable) {
                    button = aButton;
                } else if (heavyAvailable) {
                    button = bButton;
                } else {
                    row.hide();
                    return;
