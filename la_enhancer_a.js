...
function createProfile() {
    ...
    if (profileName != null && profileName != "") {
        ...
        settings.push(window.top.$('#smart_ab').prop('checked'));
...
function loadProfile(profile) {
    ...
    window.top.$('#smart_ab').prop('checked', settings[42]);
}
...
var smartAbStock = null;
function tryClick(button) {
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            if (userset[s.smart_ab]) {
                if (smartAbStock === null) {
                    var units = window.top.Accountmanager.farm.current_units;
                    smartAbStock = {
                        light: parseInt(units && units.light) || 0,
                        heavy: parseInt(units && units.heavy) || 0
                    };
                }
                var smartRow = button.closest('tr');
                var aButton = smartRow.children("td").eq(9).children("a");
                var bButton = smartRow.children("td").eq(10).children("a");

                if (smartAbStock.light >= 1 && aButton.html() != undefined) {
                    button = aButton;
                    smartAbStock.light--;
                } else if (smartAbStock.heavy > 100 && bButton.html() != undefined) {
                    button = bButton;
                    smartAbStock.heavy--;
                } else {
                    window.top.UI.InfoMessage("LC used up and HC reserve reached, switching villages...", 800);
                    if (cansend && filtersApplied) {
                        getNewVillage("n");
                    }
                    return;
...
function selectMasterButton(row) {
    var buttonToClick;
    var p1 = window.top.$.jStorage.get(sKey("profile:" + keyPressSettings.priorityOneProfile));
    var p2 = window.top.$.jStorage.get(sKey("profile:" + keyPressSettings.priorityTwoProfile));
    var p3 = window.top.$.jStorage.get(sKey("profile:" + keyPressSettings.priorityThreeProfile));
    var aButton = row.children("td").eq(9).children("a");
    var bButton = row.children("td").eq(10).children("a");
    var cButton = row.children("td").eq(11).children("a");
